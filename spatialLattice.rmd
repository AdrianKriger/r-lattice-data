...
Welcome to this exercise which explores **_Lattice Data_ with R**. 
...
&nbsp;&nbsp;&nbsp;**10. three `spgwr` significance tests**  
...
<div class="alert alert-info"> <strong>HINT! </strong> What conclusion/s can you draw about the data distribution in the previous two plots? Motivate your answer.
...
The analytical approach to the `Moran’s I` benefits from being fast. But it may be sensitive to irregularly distributed polygons. A safer approach to _**hypothesis testing**_ is to run a Monte-Carlo (MC) simulation using the `moran.mc()` function. The `moran.mc()` takes an extra argument `n`, the number of simulations
...
A regression model gives us an idea of how _different factors are associated with_ the target variable (`CRIME`) and their interaction.
...
A `p-value` close-to 0 generally indicates significance while a 0 < `p-value` > 0 typically indicates no significance.

<br>

**b. Coefficient Estimates.**
The `Estimate` indicates how the rate of change, for one unit of the predictor variable (`AVMEDINC`, `GV2015VAL`, etc.), are associated with the independent variable (`CRIME`)
...
ols <- lm(coct$CRIME ~ coct$AVMEDINC + coct$GV2015VAL  + coct$AVAPREArea + coct$AVBACH,
           na.action=na.exclude)
```
...
moran.test(residuals(ols), coct.wts, alternative="two.sided", 
...
- **Question 16.** Execute the following three a. b and c `code chunks / blocks` which summaries the OLS residuals and generates a number of `plot()`s.
...
**Finally we can compare the standardized residuals with the `t-distribution` _(the fitted values)_**
...
c. The standardized residuals vs `t-values` (the fitted values):  
] brackets ]
</p>
...
<div class="alert alert-block alert-success"> **What is the purpose of regression analysis:** _we want to predict_ 

Lets create a 2025 synthetic dataset that represents an increase in $5\%$ household income (`AVMEDINC`) and an increase of $11\%$ and $7\%$ in the value of properties (`GV2015VA`) and Bachelor degrees (`AVBACH`) and then predict how `CRIME` will change.

</div>

```{r predict-ols }
test <- coct[,c(9, 13, 18, 20)]
...
test$AVBACH <- test$AVBACH * (100 + 7)/100

# predict using the model
predCrime = round(predict(ols, newdata = test), 3)
...
#- rename
names(test)[names(test) == 'GV2015VAL'] <- 'GV2025VAL'
names(test)[names(test) == 'AVMEDINC'] <- 'AVMEDINC2025'
names(test)[names(test) == 'AVBACH'] <- 'AVBACH2025'
...
- **Question 14.** Discuss the the result. Your answer should refer to the `Coefficients` and `p-values` of the OLS regression mode. Also comments on the empty rows.
...
<div class="alert alert-block alert-success"> **CONCEPT CHECK:** 

We have **_infered_** and **_predicted_**. We have established a connection and relationship (between dependent variables: `AVMEDINC` and `GV2015VAL` and an independent variable: `CRIME`) and used the knowledge to imagine a future instance. We have not determined **_causality_**. We do not know what is _causing_ the variations in `CRIME`
</div>
...
All the previous functions accepted a `zero.policy = TRUE/FALSE` and  `na.action = na.exclude/pass` which allowed us to work around not having a complete data set (`NaN` and `NA` values). 
...
# GWR with Gauss
crime.bw01 <- gwr.sel(CRIME ~ AVMEDINC + GV2015VAL + AVAPREArea + AVBACH,
                  data=intp_coct.sp)# 
```
...
```{r bw01-fit }
gwr.fit1 <- gwr(CRIME ~ AVMEDINC + GV2015VAL+ AVAPREArea + AVBACH,
                data = intp_coct.sp, 
...
crime.bw03 <- gwr.sel(CRIME ~ AVMEDINC + GV2015VAL + AVAPREArea + AVBACH,
                  data=intp_coct.sp, 
                  adapt = TRUE)
```
...
- **Question 15.** Execute the following three `code chunks / blocks` and discuss the result.


```{r fit-adapt }
gwr.fit3 <- gwr(CRIME ~ AVMEDINC + GV2015VAL + AVAPREArea + AVBACH,
                data = intp_coct.sp, 
...
gwr.fit3
```
**While GWR does provide a comparison with a global OLS coefficients; lets increase the precision and look at the numbers more closely.**

```{r }
# table of GWR coefficients
t1 = apply(gwr.fit3$SDF@data[, 2:6], 2, summary)
# OLS coefficients
t2 = coef(ols)
...
**We can also see how `adapt` harvested different search distances across our study area** 
...
- **Question 16.** Discuss the result of `gwr.fit3$bandwidth` and the `plot()` above. Is the adaptive bandwidth more representative than a fixed distance for the entire study area?
...
- **Question 17.** Execute the following three `code chunks / blocks` and discuss the results.
...
#intp_coct$AVMEDINC.t <- gwr.fit3$SDF$AVMEDINC/gwr.fit3$SDF$AVMEDINC_se
intp_coct$GV2015VAL.t <- gwr.fit3$SDF$GV2015VAL/gwr.fit3$SDF$GV2015VAL_se
#intp_coct$AVAPREArea.t <- gwr.fit3$SDF$AVAPREArea/gwr.fit3$SDF$AVAPREArea_se

#- calculate the pvalue using the pt()
#intp_coct$AVMEDINC.t.p<-2*pt(-abs(intp_coct$AVMEDINC.t), dfree)
intp_coct$GV2015VAL.t.p<-2*pt(-abs(intp_coct$GV2015VAL.t), dfree)
#intp_coct$AVAPREArea.t.p<-2*pt(-abs(intp_coct$AVAPREArea.t), dfree)
```


```{r plot-p.t }
plot(intp_coct['GV2015VAL.t.p'], 
     main = 'GV2015VAL p-value')
```
...
<p class="comment">
[ Answer 17. click in this cell and type your answer here. your answer must be between the outer [] brackets ]
</p>

<div class="alert alert-danger">
  <strong>BONUS MARKS!</strong> </div>

- What do the `p-value` suggest for `AVMEDINC` and / or `AVAPREArea`?
...
- **Question 18.** Execute the following three `code chunks / blocks` that tests GWR and discuss the result.
...
<div class="alert alert-info"> <strong>HINT: </strong> Your answer should include a discussion highlighting the challenges associated with GWR
</div>  

</div>

<p class="comment">
[ Answer 18. click in this cell and type your answer here. your answer must be between the outer [] brackets with 50-to-150 words ]
</p>
...
p <- plot_ly(
  ...
  text = ~paste(round(CRIME), " residential theft cases in\n", OFC_SBRB_N,
                 "\n where the household income is R", round(AVMEDINC)),#, " per year"),
  hoveron='fills',
  hoverinfo = "text"
  #alpha = 0.8) %>% 
