...
Lets create a 2025 synthetic dataset that represents an increase in $5\%$ household income (`AVMEDINC`) and an increase of $11\%$ in the value of properties (`GV2015VA`) then predict how `CRIME` will change.
...
# GWR with Gauss
crime.bw01 <- gwr.sel(CRIME ~ AVMEDINC + GV2015VAL, 
                  data=intp_coct.sp)# 
```
...
```{r bw01-fit }
gwr.fit1 <- gwr(CRIME ~ AVMEDINC + GV2015VAL,
                data = intp_coct.sp, 
...
crime.bw03 <- gwr.sel(CRIME ~ AVMEDINC + GV2015VAL, 
                  data=intp_coct.sp, 
                  adapt = TRUE)
```
...
```{r fit-adapt }
gwr.fit3 <- gwr(CRIME ~ AVMEDINC + GV2015VAL,
                data = intp_coct.sp, 
...
t1 = apply(gwr.fit3$SDF@data[, 2:4], 2, summary)
# OLS coefficients
t2 = coef(reg1)
...
plot(intp_coct['GV2015VAL.t.p'], #intp_col['bwadapt'], 
     main = 'GV2015VAL p-value')#,#, border="lightgray", main=NA)
