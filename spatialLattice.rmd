...
main_map <- 
  ...
  theme_void() +
  theme(
    legend.justification = c(0, 1), #c(0, 1),
...
GWR _**does not**_ accept `NaN` and `NA` values. We either need to cull those rows of interpolate values. Since this is 
</div>
...
#intp_col$CRIME <- na.approx(intp_col$CRIME, na.rm = FALSE)
#intp_col$AVMEDINC <- na.approx(intp_col$AVMEDINC, na.rm = FALSE)
#intp_col$GV2015VAL <- na.approx(intp_col$GV2015VAL, na.rm = FALSE)
```
...
# GWR with Gauss
crime.bw01 <- gwr.sel(intp_col$CRIME ~ intp_col$AVMEDINC + intp_col$GV2015VAL, 
...
```{r bw01-fit }
gwr.fit1 <- gwr(intp_col$CRIME ~ intp_col$AVMEDINC + intp_col$GV2015VAL,
                data = intp_col, 
