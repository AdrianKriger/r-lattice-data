...
coct_sub <- coct[,c(4, 9, 12:14, 17:18, 20)]
...
- **Question 6.** Execute the next three `code chunks / blocks` and comment on what you observe.
...
- **Question 10.** NOTICE: The `0.8` observed statistic falls all the _way_ to the right. What does this mean? 
...
plot(coct['coct_locm.Ii'], main = 'Local Morans statistic (Ii)')#,#, border="lightgray", main=NA)
     #col = col_locm[,1])
```
...
- **Question 14.** What conclusion can you draw from the `Multiple R-`, `Adjusted R-squared` and `p-` values? Additionally comment on the _`sign`_ of the independent variables. 
...
- **Question 15.** Is there autocorrelation in the residuals? Substantiate your answer.
...
#- bivariate Moran's I
#- neighbourhood with queen's case:
intp_coct.nb <- poly2nb(intp_coct, queen = T)

#Define a network grid
intp_coct.wts <- nb2listw(intp_coct.nb, style="W")

x <- intp_coct$CRIME
y <- intp_coct$GV2015VAL
nb <- intp_coct.nb
wt <- intp_coct.wts
...
# GWR with Gauss
crime.bw01 <- gwr.sel(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col.sp$GV2015VAL,# + AVESTADist, 
