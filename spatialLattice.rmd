...
This assignment is a brief tutorial on the _**spatial regression**_ functionality contained in the `R` package `spdep`. We will to start to learn some of the methods necessary to determine _**spatial autocorrelation**_ and later continue onto the `spgwr` package which explores _**geographically weighted regression**_.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; •	`AvePrecinctsLen` : average precinct perimetre  
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; •	`AVBACH` : avergae number bachelors degrees  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; •	`STA` : police station  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; •	`STAD` : distances to the local police station  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; •	`PREA` : precinct area  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; •	`PREL` : precinct perimetre  
...
- **Question 3a.** Create a subset data from the coct (select relevant variables and generate summary statistics etc.). Select the numerical variables in columns 7 to 12 and store the subset data into coct_sub variable. 

```{r answer3}
##-- click in this cell and type your answer here. your answer must be between the ``` backticks ```

coct_sub <- col[,c(4, 9, 12:14, 17:18, 20)]
...
- **Question 6.** Execute the next three `code chunks / blocks` and comment on what you observe. What conclusion/s can you draw about the data distribution in the previous three plots? Motivate your answer.

```{r plot}
#- plot
plot(col["CRIME"], lwd = 0.2)#, axes = TRUE)
```
...
<!-- #     name = "Residential Crime per Suburb" -->
<!-- #     ) + -->
<!-- #   theme_void() + -->
<!-- #   theme( -->
...
<!-- #     legend.position = c(0.9, .92) -->
<!-- #     ) + -->
<!-- #   theme( -->
<!-- #     text = element_text(family = "Palatino-Bold"), -->
<!-- #     legend.title = element_text(family="Palatino-Bold", size = 10), #element_text(family = "Candara-Bold", size = 10), -->
<!-- #     legend.text = element_text(family = "Palatino-Medium", size = 10) -->
<!-- #     ) -->
<!-- # ``` -->

```{r ggplot-with-inset}
## Make main map object --not correct [https://upgo.lab.mcgill.ca/2019/12/13/making-beautiful-maps/]

main_map <- 
  col %>% 
  ggplot() +
  #ggtitle("2015 Residential theft per suburb \n City of Cape Town") +
  geom_sf(
    aes(fill = CRIME),# / dwellings),
    lwd = 0.1, 
    colour = "white"
    ) +
  geom_rect(
  ...
  scale_fill_gradientn(
    ...
    oob = scales::squish,
    #labels = scales::percent,
    name = "2015 Residential Crime (per suburb)"
    ) +
  coord_sf(expand = FALSE) +
  theme_void() +
  theme(
    legend.justification = c(-0.2,1), #c(0, 1),
    legend.position = 'right' # #c(0.7, 0.8)#
    ) +
  theme(
    text = element_text(family = "NimbusMon", face="bold"),
    legend.title = element_text(family = "NimbusMon", face="bold", size = 10),
    legend.text = element_text(family = "NimbusMon", face="plain", size = 10)
    )  
  ...
  draw_plot(
    {
      main_map + 
        coord_sf(
          xlim=c(255000,290000),
          ylim=c(6235000,6247000),
          expand = FALSE) +
    ...
    height = 0.35)
#)
#main_map %>% ggtitle("2015 Residential theft per suburb \n City of Cape Town")
```

```{r plot-binary}
# visualizing a variable - Binary classification
plot(col['CRIME'], main = NA, #"City of Cape Town: Residential Crime, 2015 \n (cases per suburb)", 
     col = ifelse(col$CRIME > 400, 
                                              "lightgrey", "red"))
...
This is an excellent opportunity to highlight the challenges with using choropleth maps. Choropleth maps often give a false impression of an abrupt change at the boundaries of shaded units. These types of maps are also not suitable for showing total values.
...
# create class breaks 
brks.eq = classIntervals(col$CRIME, n = 7, style = "equal")
brks.qt = classIntervals(col$CRIME, n = 7, style = "quantile")
brks.jk = classIntervals(col$CRIME, n = 7, style = "jenks")
...
plot(col['CRIME'],  col=brks.eqcol, lwd = 0.1, main=NA)
     #border=NA,  #"black")
...
plot(col['CRIME'],  col=brks.qtcol, lwd = 0.1, main=NA)
     #border=NA,  #"black") 
legend("topleft", leglabs(round(brks.eq$brks, digits=1)), 
       fill=pal, cex=0.8, bty="n")
title(main="City of Cape Town: Residential Crime per suburb, 2015 \n (Quantile breaks)")
...
```{r plot-jenks}
# Plot with Equal Breaks
plot(col['CRIME'],  col=brks.jkcol, lwd = 0.1, main=NA)
     #border=NA,  #"black")
...
In addition, the degree to which the neighbors’ attributes are related to a given suburb or not can be computed _(i.e. local spatial autocorrelation)_.
...
col.nb <- poly2nb(col, queen = T)

#Define a network grid
col.wts <- nb2listw(col.nb, zero.policy = TRUE, style="W")
```
...
print(col.wts, zero.policy=TRUE)  
```
...
Spatial autocorrelation captures the degree to which near and distant things are related. We want to know how variables are connected and their relationship. 
...
<br>
What this means is; if we where predicting elevation or crop yield or the prevalence of malaria the **_general trend_** could be captured with all the terms containing $𝜷$ **_without accounting for a spatially autocorrelated random processes_**, such as localized topographic changes, clusters of different management practices or regional prevention and treatment campaigns.

When our observations are spatially autocorrelated our predictions will be false and we have to restructure our residuals ($ϵ$) with a variogram. _(see [r-spatial-stats](https://adriankriger.github.io/r-spatial-stats/) next week)_
...
The most common method for testing _**spatial autocorrelation**_ is by using the Global Moran’s I test. This method **compares how similar each polygon is with its neighbours** and averages all these comparisons. The result is one value that will yield the overall impression of how a given variable is correlated with itself across the study area
...
The easy way to calculate Moran’s I statistic is simply though the `moran.test` function. We execute the function with `zero.policy = TRUE` and `na.action = na.pass` to account for no-data values while `randomisation = T` tells us that the attribute being analyzed (crime in the City of Cape Town) is randomly distributed _(Is it? ---what does the histogram above say?)_.
...
#crs(col.net) <- st_crs(col)

globalMoran <- moran.test(col$CRIME, col.wts, zero.policy = TRUE,  
                     randomisation = T, 
...
The analytical approach to the Moran’s I benefits from being fast. But it may be sensitive to irregularly distributed polygons. A safer approach to _**hypothesis testing**_ is to run an MC simulation using the `moran.mc()` function. The `moran.mc()` takes an extra argument `n`, the number of simulations
...
#globalMoran <- moran.test(col$CRIME, col.wts, zero.policy = TRUE,  
                     #randomisation = T, 
                     #na.action = na.pass)#, n = m, S0 = s) #
MC <- moran.mc(col$CRIME, col.wts, zero.policy = TRUE,  
...
**We can `plot` the distribution of the `MC` methods Moran I values we expect had crime been randomly distributed across the suburbs** 
...
- **Question 9.** NOTICE: The `0.8` observed statistic falls all the way to the right. What does this mean? 
...
We can visualize Morans statistic with a scatterplot. The plot places our variable of interest on the x-axis (crime) and the spatially weighted sum of neighbor values (i.e. spatially lagged values) on the y axis. We can think of the Global Moran’s I as a linear relationship between these (drawn as a slope in the figure).

```{r moran-scatter }
# moran Scatterplot

#colNA = col[!is.na(col$CRIME),]
#colNA.nb = poly2nb(colNA, queen = T)  
#colNA.wts <- nb2listw(colNA.nb, style = "W", zero.policy = TRUE)

colsmp <- moran.plot(col$CRIME, col.wts, zero.policy = TRUE,
                     labels = as.character(col.wts$OBJECTID),#OBJECTID), 
                     xlab = "CRIME", ylab = "Lag of CRIME")
```
<div class="alert alert-warning"> <strong>QUESTION!</strong> </div>

- **Question 9.** What is the purpose of the Global Moran I scatter plot? What does it represent? Discuss the Global Moran's I scatter plot above.
...
```{r correl-plot }
#- correlogram
col_cor8 <- sp.correlogram(col.nb, col$CRIME, order = 8, 
                           method = "I", zero.policy = TRUE)
#col_cor5 
#- plot correlogram
plot(col_cor8, main = "Correlelogram of CRIME")
```
<div class="alert alert-warning"> <strong>QUESTION!</strong> </div>

- **Question 10.** What conclusion/s can you draw from the correlogram plot? 
...
Like Global Moran’s I, the **Local Moran’s I statistic** can be used to _quantify spatial patterns occurring across a given area_. However, instead of producing one value that encompasses the entire study area, the **Local Moran’s I method produces a statistic for each polygon in the study area**.

The advantage of this localized method is that it shows exactly which polygons are similar or different to the objects in their neighborhood. In other words, it identifies where spatial autocorrelation occurs as well as the level of significance that is experienced.

```{r local-moran }
#- local Moran's I
col_locm <- localmoran(col$CRIME, col.wts, zero.policy = TRUE,  
                     na.action = na.pass)
summary(col_locm)
```

We get a number of useful statistics from the local model:
...
We can plot values these to visualize how and the degree _(strength)_ of the connections / relationships. 
...
- **Question 11.** Execute the following `code chunk / block` and discuss the result. 
```{r plot-local-moran-clusters }
quadrant <- vector(mode="numeric", length=nrow(col_locm))

# centers the variable of interest around its mean
m.crime <- col$CRIME - mean(col$CRIME)     

# centers the local Moran's around the mean
m.local <- col_locm[,1] - mean(col_locm[,1])    
...
quadrant[col_locm[,5]>signif] <- 0   

# plot in r
brks <- c(0,1,2,3,4)
colors <- c("white", "blue", rgb(0,0,1, alpha=0.4), rgb(1,0,0,alpha=0.4), "red")
plot(col['CRIME'], border="lightgray", main=NA,
...
```{r plot-moran-Ii }
col$col_locm.Ii <- col_locm[,1]
plot(col['col_locm.Ii'], main = 'Local Morans statistic (Ii)')#,#, border="lightgray", main=NA)
     #col = col_locm[,1])
```

```{r scale-crime }
CRIME <- scale(col$CRIME) #%>% as.vector()
summary(CRIME)
```

```{r }
col$lag_sCRIME <- lag.listw(col.wts, CRIME, zero.policy = TRUE, NAOK = TRUE)
summary(col$lag_sCRIME)
```
...
Normally we take a dependent variable (crime) and try and explain the variation scores using an independent variable or a suite of uncorrelated and normally distributed independent variables (household income. property valuation). The strength and direction of association is indicated by the regression coefficients, with one coefficient given for each variable in the dataset.

Lets execute a basic OLS regression for crime per suburb on the following independent variables: Property Valuation and Income.

```{r ols-r }
# OLS Regression to predict CRIME using AVEMEDINC and GV2015VAL as independent variables
# https://crd230.github.io/gwr.html#ordinary_least_squares_regression
reg1 <- lm(col$CRIME ~ col$AVMEDINC + col$GV2015VAL, na.action=na.exclude)
```
...
- **Question 12.** What conclusion can you draw from the `Multiple R-`, `Adjusted R-squared` and `p-` values? Additionally comment on the _`sign`_ of the independent variables. 
...
moran.test(residuals(reg1), col.wts, alternative="two.sided", 
           zero.policy = TRUE,  na.action = na.exclude)
```

<div class="alert alert-warning"> <strong>QUESTION!</strong> </div>

- **Question 13.** Is there autocorrelation in the residuals? Substantiate your answer.
...
plot(reg1, which=3)
```
**Similar to the LISA cluster; the `OLS residuals` however better describe whether a spatial pattern exists or not** 
...
par(mar=rep(0,4))
plot(col['CRIME'], col=cols, main="Residuals from OLS Model")#, border="grey")
...
All the previous function accepted a `zero.policy = TRUE` and / or  `na.action = na.exclude` which allowed us to work around not having a a complete data set (`NaN` and `NA` values). Geographically Weighted Regression (GWR) _**does not**_ accept `NaN` and `NA` values. In order to proceed onto GWR we either need to cull those rows or interpolate values. Since this exercise aims to demonstrate concepts and potential we interpolate. **This should not be standard practice.** 
...
coct_sub <- col[,c(4, 9:14, 17:18, 20, 26)]

#md.pattern(coct_sub, rotate.names = TRUE)
...
```{r impute }
intp_col <- coct_sub
intp_col$GV2015VAL <- na.approx(coct_sub$GV2015VAL, rule = 2)
summary(aggr(intp_col))
```
...
<!-- #intp_col$AVBACH <- na.approx(intp_col$AVBACH, na.rm = FALSE) -->
<!-- #intp_col$AVESTADist <- na.approx(intp_col$AVESTADist, na.rm = FALSE) -->
<!-- #intp_col$AVAPREArea <- na.approx(intp_col$AVAPREArea, na.rm = FALSE) -->
<!-- ``` -->
...
nb <- col.nb
wt <- col.wts
local_moran_bv(x, y, nb, wt, zero.policy = TRUE)
```
...
#sf_cent <- st_centroid(col['geom'])
#xy = st_coordinates(sf_cent)
intp_col.sp <- as(intp_col, "Spatial")
# GWR with Gauss
crime.bw01 <- gwr.sel(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col$GV2015VAL,# + AVESTADist, 
                  data=intp_col.sp)#, #adapt = TRUE, # data=col,
                  #coords = cbind(intp_col.sp$X, intp_col.sp$Y))#cbind(xy[,1], xy[,2]))#
```
...
```{r bw01-fit }
gwr.fit1 <- gwr(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col.sp$GV2015VAL,# + AVESTADist,
                data = intp_col.sp, 
...
gwr.fit1
```
We can increase the change the weighting function to `bi-square` and `fit` another `regression` to compare.

<div class="alert alert-warning"> <strong>TASK!</strong> </div>

- **Question 14.** Execute the following two `code chunks / blocks` and discuss the result. 


```{r bi-sqaure }

crime.bw02 <- gwr.sel(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col.sp$GV2015VAL, 
                  data=intp_col.sp, 
                  gweight = gwr.bisquare)#, # data=col,
                  #coords = cbind(intp_col$X, intp_col$Y))#cbind(xy[,1], xy[,2]))#

gwr.fit2 <- gwr(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col.sp$GV2015VAL,
                data = intp_col.sp, 
                #coords = cbind(intp_col$X, intp_col$Y),
                bandwidth = crime.bw02, gweight = gwr.bisquare,
...
<div class="alert alert-info"> <strong>HINT: </strong> How does `crime.bw01` compare with `crime.bw02`.  How does `gwr.fit1` compare with `gwr.fit2`.  
...
The GWR models we ran above yielded a fixed distance to search for neighbors to include in the local regression. But there are places in our data where suburbs are more densely occurring. This means that in some areas, specifically along Voortrecker Rd. heading east and Main Rd. heading south, we'll include a larger number of neighboring suburbs in the OLS compared to other areas, such as large areas on the periphery of the city’s boundaries.

In this case, an adaptive kernel is suitable.

```{r adapt }

crime.bw03 <- gwr.sel(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col.sp$GV2015VAL, 
                  data=intp_col.sp, 
                  adapt = TRUE)#, # data=col,
                  #coords = cbind(intp_col$X, intp_col$Y))#cbind(xy[,1], xy[,2]))#
```
...
```{r fit-adapt }
gwr.fit3 <- gwr(intp_col.sp$CRIME ~ intp_col.sp$AVMEDINC + intp_col.sp$GV2015VAL,
                data = intp_col.sp, 
                #coords = cbind(intp_col$X, intp_col$Y),
...
```{r plot-gwr-adapt }
intp_col$bwadapt <- gwr.fit3$bandwidth

plot(intp_col['bwadapt'], #intp_col['bwadapt'], 
     main = 'GWR bandwidth')#,#, border="lightgray", main=NA)
...
intp_col.sp$GV2015VAL.t <- gwr.fit3$SDF$intp_col.sp.GV2015VAL/gwr.fit3$SDF$intp_col.sp.GV2015VAL_se

intp_col$GV2015VAL.t.p<-2*pt(-abs(intp_col.sp$GV2015VAL.t), dfree)
```

```{r plot-p }
#intp_col$bwadapt <- gwr.fit3$bandwidth

plot(intp_col['GV2015VAL.t.p'], #intp_col['bwadapt'], 
     main = 'p-value')#,#, border="lightgray", main=NA)
     #col = intp_col['bwadapt'])
```
<div class="alert alert-warning"> <strong>TASK!</strong> </div>

- **Question 16.** What do the results suggest about the spatial heterogeneity of the relationship between the 2015 property valuation (`GVAL2015`) and residential theft? (`CRIME`)?
...
<div class="alert alert-info"> <strong>HINT: </strong> How does the `p-value` correspond with the LISA
