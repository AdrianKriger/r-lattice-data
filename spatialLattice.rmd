...
coct_sub <- col[,c(4, 9:12, 15:16, 18)]
...
GWR _**does not**_ accept `NaN` and `NA` values. We either need to cull those rows or interpolate values. Since this exercise aims to demonstrate concepts and potential we interpolate. **This should not be standard practice.** 
...
# GWR with Gauss
crime.bw01 <- gwr.sel(intp_col$CRIME ~ intp_col$AVMEDINC + intp_col$GV2015VAL,# + AVESTADist, 
...
```{r bw01-fit }
gwr.fit1 <- gwr(intp_col$CRIME ~ intp_col$AVMEDINC + intp_col$GV2015VAL,# + AVESTADist,
                data = intp_col, 
...
crime.bw02 <- gwr.sel(intp_col$CRIME ~ intp_col$AVMEDINC + intp_col$GV2015VAL, 
                  data=intp_col, gweight = gwr.bisquare, # data=col,
                  coords = cbind(intp_col$X, intp_col$Y))#cbind(xy[,1], xy[,2]))#

gwr.fit2 <- gwr(intp_col$CRIME ~ intp_col$AVMEDINC + intp_col$GV2015VAL,
                data = intp_col, 
                coords = cbind(intp_col$X, intp_col$Y),
